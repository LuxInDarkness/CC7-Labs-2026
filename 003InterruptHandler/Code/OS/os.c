...
// QEMU VersatilePB does not have a timer, so we will skip timer functions
void timer_init(void) {
    // No timer on VersatilePB
}

/* No IRQs on VersatilePB */
...
void timer_init(void) {
    // TODO: Verify timer initialization
...
// 3. Print "Tick\n" via UART
void timer_irq_handler(void) {
    // TODO: Verify timer interrupt handler
    PUT32(TISR, 0x2);
...
int main(void) {
    ...
    os_write("Initializing...\n");

#if defined(PLATFORM_VERSATILEPB)
    os_write("Running on VersatilePB (no timer interrupts)\n");
#else
    timer_init();
    enable_irq();
    os_write("Interrupts enabled. Waiting for timer ticks...\n");
#endif
